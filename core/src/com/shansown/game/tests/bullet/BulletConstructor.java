...
public class BulletConstructor extends BaseWorld.Constructor<BulletEntity> {
	...
	@Override
	public BulletEntity construct (final float x, final float y, final float z,
								   final float angleX, final float angleY, final float angleZ) {
		if (bodyInfo == null && shape != null) {
			btCollisionObject obj = new btCollisionObject();
			obj.setCollisionShape(shape);
			return new BulletEntity(model, obj, x, y, z, angleX, angleY, angleZ);
		} else
			return new BulletEntity(model, bodyInfo, x, y, z, angleX, angleY, angleZ);
	}
