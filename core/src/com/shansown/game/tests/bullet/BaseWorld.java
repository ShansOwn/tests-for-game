...
public class BaseWorld<T extends BaseEntity> implements Disposable {
	...
	public T add (final String type, float x, float y, float z, float angleX, float angleY, float angleZ) {
		final T entity = constructors.get(type).construct(x, y, z, angleX, angleY, angleZ);
		add(entity);
		return entity;
	}
